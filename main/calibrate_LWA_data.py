...
stand_cable_delays = NP.loadtxt('/data3/t_nithyanandan/project_MOFF/'
                                'data/samples/cable_delays.txt', skiprows=1)
...
if use_GSM:
    ...
    beam_interp = interpolate.griddata((smalll, smallm), smallb,
                                       (xyz[0, :], xyz[1, :]), method='linear')
    ...
    # Values retrieved from http://www.cv.nrao.edu/vlss/VLSSlist.shtml.save
    # (sum nearby components)
    src_flux = NP.array([20539.0, 19328.0])
    # Manually adjusting by a rough factor of the beam because the cal module doesn't do it (yet)
    src_flux[1] = src_flux[1] * 0.57
...
for pol in pols:
    ...
    if scramble_gains > 0:
        for i in NP.arange(NP.ceil(NP.float(nchan) / freq_ave)):
            ...
            calarr[pol].curr_gains[:, mini:maxi] += (NP.random.normal(0, NP.sqrt(scramble_gains), (n_antennas, 1)) +
                                                     1j * NP.random.normal(0, NP.sqrt(scramble_gains), (n_antennas, 1)))
...
for i in xrange(max_n_timestamps):
    ...
    if test_sim:
        ...
        E_timeseries_dict = SIM.stochastic_E_timeseries(f_center, nchan / 2, 2 * channel_width,
                                                        ...
                                                        antpos=antpos_info['positions'], tshift=False)
    ...
    for ia, label in enumerate(antid):
        ...
        for ip, pol in enumerate(pols):
            ...
            adict['wtsinfo'][pol] = [{'orientation': 0.0, 'lookup': '/data3/'
                                      't_nithyanandan/project_MOFF/simulated/LWA/'
                                      'data/lookup/E_illumination_isotropic_radiators_lookup_zenith.txt'}]
    ...
    if grid_map_method == 'regular':
        aar.grid_convolve_new(pol='P1', method='NN', distNN=0.5 * NP.sqrt(ant_sizex**2 + ant_sizey**2),
                              identical_antennas=False, cal_loop=False, gridfunc_freq='scale',
                              wts_change=False, parallel=False, pp_method='pool')
    else:
        if i == 0:
            aar.genMappingMatrix(pol='P1', method='NN', distNN=0.5 * NP.sqrt(ant_sizex**2 + ant_sizey**2),
...
xlabel('l')
ylabel('m')
cb = colorbar(orientation='horizontal', ticks=[0, 1200, 2400, 3600], pad=.1,
