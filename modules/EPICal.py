...
class cal:
    ...
    *** Functions ***
    ...
    def __init__(self, freqs, ant_pos, ref_ant=0, freq_ave=1, pol='P1', curr_gains=None,
                 sim_mode=False, n_iter=10, damping_factor=0.0, inv_gains=False,
                 sky_model=NP.ones(1, dtype=NP.float32), cal_sources=None, n_cal_sources=1,
                 phase_fit=False, auto_noise_model=0.0, exclude_autos=False,
                 fix_holographic_phase=True, flatten_array=False, conv_thresh=0.000001,
                 conv_max_try=2000):
        ...
        if (damping_factor > 1.0) or (damping_factor < 0.0):
            damping_factor = 0.0
    ...
    def scramble_gains(self, amp):
        # Mix up gains a bit
        self.curr_gains += (NP.random.normal(0, NP.sqrt(amp), self.curr_gains.shape) +
                            1j * NP.random.normal(0, NP.sqrt(amp), self.curr_gains.shape))
    ...
    def update_model_vis(self, gridl, gridm):
        ...
        for i in NP.arange(self.n_cal_sources):
            xind, yind = NP.unravel_index(NP.argmin((gridl -
                                          self.sky_model[self.cal_sources[i], 0, 0])**2 +
                                          (gridm - self.sky_model[self.cal_sources[i], 0, 1])**2),
                                          gridl.shape)
            self.cal_pix_inds[i, :] = NP.array([xind, yind])
            self.cal_pix_locs[i, :] = NP.array([gridl[xind, yind], gridm[xind, yind],
                                                NP.sqrt(1 - gridl[xind, yind]**2 -
                                                gridm[xind, yind]**2)])
        ...
        antnum = 0
        p = PGB.ProgressBar(widgets=[PGB.Percentage(), PGB.Bar(marker='-', left=' |', right='| '),
                                     PGB.Counter(), '/{0:0d} Antennas '.format(self.n_ant),
                                     PGB.ETA()], maxval=self.n_ant).start()
        for ant in xrange(self.n_ant):
            if self.sky_model.shape[1] > 1:
                model_vis[ant, :, :] = NP.sum(model_flux * NP.exp(-1j * 2 * NP.pi *
                                              NP.sum(model_pos * (ant_pos - NP.reshape(ant_pos[:, ant, :, :], (1, 1, self.n_chan, 3))), axis=3)), axis=0)
            else:
                model_vis[ant, :, :] = NP.sum(model_flux * NP.exp(-1j * 2 * NP.pi *
                                              NP.sum(model_pos *
                                                     (ant_pos[:, :, self.n_chan / 2, :].reshape(1, self.n_ant, 1, 3) -
                                                      NP.reshape(ant_pos[:, ant, self.n_chan / 2, :], (1, 1, 1, 3))),
                                                     axis=3)), axis=0)
            p.update(antnum + 1)
            antnum += 1
        p.finish()
        ...
        if self.flatten_array:
            for ant in xrange(self.n_ant):
                ...
                model_vis[ant, :, :] *= NP.exp(1j * 2 * NP.pi * (self.ant_pos[:, :, 2] -
                                                                 self.ant_pos[ant, :, 2].reshape(1, -1)))
    ...
    def update_cal(self, Edata, imgobj):
        ...
        for i in xrange(self.n_cal_sources):
            imgdata[i, :] = imgobj.holimg[self.pol][self.cal_pix_inds[i, 0],
                                                    self.cal_pix_inds[i, 1], :].flatten()
        ...
        if self.count == self.n_iter:
            ...
            temp_gains = NP.repeat(self.curr_gains.reshape(1, self.n_ant, self.n_chan),
                                   self.n_cal_sources, axis=0)
            tries = 0
            ...
            if self.fix_holographic_phase:
                # The holographic images have a silly phase running through them
                # due to not centering when padding. Take it out.
                dl = imgobj.gridl[0, 1] - imgobj.gridl[0, 0]
                dm = imgobj.gridm[1, 0] - imgobj.gridm[0, 0]
                phase_fix = NP.exp(-1j * NP.pi * (self.cal_pix_locs[:, 0] / dl +
                                                  self.cal_pix_locs[:, 1] / dm) /
                                   2).reshape(self.n_cal_sources, 1, 1)
            ...
            if self.exclude_autos:
                ...
                self.cal_corr = self.cal_corr - (NP.exp(1j * 2 * NP.pi *
                                                        NP.sum(self.ant_pos[:, :, 0:2].reshape((1, self.n_ant,
                                                               self.n_chan, 2)) *
                                                               self.cal_pix_locs[:, 0:2].reshape(self.n_cal_sources, 1, 1, 2), axis=3)) *
                                                 self.auto_corr.reshape((1, self.n_ant, self.n_chan)) *
                                                 NP.conj(applied_cal) / self.n_ant)
            ...
            while (tries < self.conv_max_try) and (change > self.conv_thresh):
                ...
                temp_gains = (self.cal_corr * (NP.sum(self.ant_twt, axis=0).reshape(1, 1, -1) -
                                               self.ant_twt.reshape(1, self.n_ant, self.n_chan)) *
                              self.n_ant / NP.sum((self.n_ant - 1) *
                              self.ant_twt.reshape(1, 1, self.n_ant, self.n_chan) *
                              NP.exp(1j * 2 * NP.pi * NP.sum(self.ant_pos[:, :, 0:2].reshape((1, 1, self.n_ant, self.n_chan, 2)) *
                                     self.cal_pix_locs[:, 0:2].reshape(self.n_cal_sources, 1, 1, 1, 2), axis=4)) *
                              self.model_vis.reshape((1, self.n_ant, self.n_ant, self.n_chan)) *
                              NP.conj(NP.reshape(applied_cal * prev_gains,
                ...
                for i in xrange(NP.int(NP.ceil(NP.float(self.n_chan) / self.freq_ave))):
                    ...
                    temp_gains[:, :, mini:maxi] = NP.nanmean(temp_gains[:, :, mini:maxi],
                                                             axis=2).reshape(self.n_cal_sources,
                                                                             self.n_ant, 1)
            ...
            self.curr_gains = NP.where(NP.isnan(temp_gains), self.curr_gains,
                                       self.curr_gains * self.damping_factor +
                                       temp_gains * (1 - self.damping_factor))
            if self.phase_fit:
            ...
            # Reset integrations
            self.count = 0
            self.cal_corr = NP.zeros((self.n_cal_sources, self.n_ant, self.n_chan),
                                     dtype=NP.complex64)
