...
def vis_cal(visdata,vismodel,max_iter=2000,threshold=0.000001,damping_factor=0.05,ref_ant=0):
	...
	for fi in xrange(nchan):
		...
		while (tries < max_iter) and (change > threshold):
			chi_history[tries,fi] = NP.sum(NP.abs(tempdata-tempgains[ind2]*NP.conj(tempgains[ind3])*tempmodel)**2)
		...
		#gains[:,fi] = gains[:,fi]*damping_factor + tempgains.copy()*(1-damping_factor)
